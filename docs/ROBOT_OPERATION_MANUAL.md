...
四旋翼默认使用 IsaacSim Crazyflie `cf2x.usd`。如果项目内没有 Isaac 资产镜像，需要通过 `--quadrotor-usd` 指向本机 USD，或通过 `--isaac-asset-root` 指向 IsaacSim 资产根目录。

默认启用 **物理 6-DOF 飞控**（`--quadrotor-physics`）：保留 Crazyflie 关节体的刚体物理（重力、碰撞、旋翼转动副），用级联 SE3 几何控制器（位置 PD → 期望推力向量；姿态 PD → 机体系力矩）驱动 `body` 刚体跟踪轨迹。暂停时无人机会在当前设定点悬停，`F4` 重置会把机体瞬移回轨迹起点并清零速度。需要回到旧的纯运动学（直接搬 prim、无飞行动力学）模式时加 `--no-quadrotor-physics`。

单机物理飞行测试（读取预设轨迹 `1.csv`）：
...
> 物理飞控建议使用 `--quadrotor-scale 1.0`：动力学使用 USD 中标注的 Crazyflie 质量/惯量（约 28 g），与视觉缩放无关；非 1.0 缩放只放大碰撞/视觉，可能造成失真。
...
`--quadrotor-count` 控制无人机数量；轨迹从 `--quadrotor-trajectory-dir` 按序号读取 `1.csv`、`2.csv` … `N.csv`。

物理飞控增益可通过 `--quadrotor-physics-kp-pos`、`--quadrotor-physics-kd-pos`、`--quadrotor-physics-k-att`、`--quadrotor-physics-k-omega`、`--quadrotor-physics-max-thrust`、`--quadrotor-physics-max-torque`、`--quadrotor-physics-rotor-spin` 调整；默认值已针对 Crazyflie 微型机体标定。

默认会打开一个外部 Matplotlib 窗口显示四旋翼 RGB-D 图像，IsaacSim viewport 继续用于场景展示。需要关闭外部窗口时添加 `--no-show-quadrotor-camera-window`；需要在 IsaacSim 内临时查看相机时，先按 `F2` 再按 `F6`。
...
四旋翼当前是运动学控制，用于先验证场景集成、RGB-D 相机和指令轨迹跟踪；不会模拟桨叶推力和飞控动力学。
...
| `F3` | 暂停或恢复轨迹 |
| `F4` | 重置轨迹到第一个航点 |
| `F6` | 在 IsaacSim viewport 中临时切换到四旋翼 RGB-D 相机 |
| `W/S` | 轨迹暂停时前进/后退 |
| `A/D` | 轨迹暂停时左/右平移 |
| `Q/E` | 轨迹暂停时下降/上升 |
| `Z/X` | 轨迹暂停时左/右偏航 |
